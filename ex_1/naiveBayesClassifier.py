...
def naiveBayesClassifier(x_train,x_test,y_train):
...
def printMatrix(c,matrix):
    ...
    plt.savefig("NaiveBayesConfusionMatrix/" + c+'.png', dpi=150)
    plt.close()
...
def main():
    ...
    for c in objectiveCols:
        ...
        naiveClassifier=naiveBayesClassifier(x_train,x_test,y_train)

        print("results of the naive Bayes classifier Index for "+ c)
        yNaive=predict(x_test,naiveClassifier,c)
        evaluation(y_test,yNaive)
        matrix=confusionMatrix=confusion_matrix(y_test, yNaive)
        printMatrix(c,matrix)

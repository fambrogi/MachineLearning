...
def nkkClassifier(x_train,x_test,y_train):
...
def printMatrix(c,matrix):
    ...
    plt.savefig("NKKConfusionMatrix/" + c+'.png', dpi=150)
    plt.close()
...
def main():
    ...
    for c in objectiveCols:
        ...
        naiveClassifier=nkkClassifier(x_train,x_test,y_train)

        print("results of the naive Bayes classifier Index for "+ c)
        yNkk=predict(x_test,naiveClassifier,c)
        evaluation(y_test,yNkk)
        matrix=confusion_matrix(y_test, yNkk)
